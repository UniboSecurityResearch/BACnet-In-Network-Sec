...
if [ "$PPT_FLAG" = true ]; then
    ...
    if [ -n "${MODE:-}" ]; then
        FILE=/shared/results_s1_"${MODE}"_"$OPTION"_packet_processing_time.txt
    else
        FILE=/shared/results_s1_"$OPTION"_packet_processing_time.txt
    fi
...
if [ "$DEQ_FLAG" = true ]; then
    ...
    if [ -n "${MODE:-}" ]; then
        FILE=/shared/results_s1_"${MODE}"_"$OPTION"_packet_dequeuing_timedelta.txt
    else
        FILE=/shared/results_s1_"$OPTION"_packet_dequeuing_timedelta.txt
    fi
    SAMPLES_COUNT="$(read_samples_count)"
